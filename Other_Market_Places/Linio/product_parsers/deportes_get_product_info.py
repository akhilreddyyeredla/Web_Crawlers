...
class ProductDetails:
    ...
    def get_data(self, product_parameters, product_link, hirearchy, dir):
        product = dict()
        ...
        product['marketplace'] = Linio_Mex_config.MARKETPLACE
        product['domain'] = product_parameters["Domain"]
        product['category'] = Linio_Mex_config.BELLEZA
        ...
        product['currency'] = Linio_Mex_config.CURRENCY
    ...
    def get_details(self, prod_url_info, raw_data):
        ...
        dirs = ''
        try:
            if DataCollectors_Configuration.DEBUG == Linio_Mex_config.ON:
                print 'directories: ', line[2:-1]
            for category in line[2:-1]:
                dirs += category + '+'

        except Exception as e:
            dirs = Linio_Mex_config.BELLEZA
        ...
        product_parameters = {
            ...
            "Marketplace": Linio_Mex_config.MARKETPLACE,  # ss
            "Description": {'Description': "not_available", },  # ss
            "Seller_Name": "not_available",  # ss
            "Domain": Linio_Mex_config.MAIN_URL,  # ss
            ...
            "Currency": Linio_Mex_config.CURRENCY,  # ss
        ...
        try:
            ...
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'product_name|'
        ...
        try:
            ...
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'price|'
            price = "not_available"
        ...
        try:
            ...
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'brand'
            brand = "not_available"

        # Retrieving image_URL of the product
        try:
            image_url = 'https:' + product_source.find_all('div', {'class': 'swiper-wrapper'})[0].find_all('img')[
                0].get(
                'data-lazy')
        except Exception as e:
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'image_url|'
        ...
        try:
            ...
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'availability|'
            availability = 'Available'
        ...
        try:
            ...
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'rating|'
            rating = "not_available"

        # Finds the seller name
        try:
            seller_name = \
                product_source.find_all('div', {'class': 'seller-information'})[0].find_all('a',
                                                                                            {'class': 'link-lower'})[
                    0].text.encode('utf8', 'ignore')
        except Exception as e:
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'seller_name'
        ...
        try:
            ...
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'seller_rating|'
        ...
        try:
            ...
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'sellers|'
        ...
        try:
            ...
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'description|'
        ...
        try:
            ...
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'varients|'
            variants = 'not_available'
        ...
        try:
            discount = product_source.find_all('span', {'class': 'discount'})[0].text.replace('-', '').encode('utf8',
                                                                                                              'ignore')
            original_price = product_source.find_all('span', {'class': 'original-price'})[0].text.replace('$',
                                                                                                          '').replace(
                ',', '').encode('utf8', 'ignore')
            if not price:
                price = "not_available"
            ...
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'discount|'
                self.print_satement = self.print_satement + 'orignal_price|'
        ...
        try:
            ...
            if DataCollectors_Configuration.DEBUG:
                self.print_satement = self.print_satement + 'specification|'
            table = "not_available"
        ...
        if DataCollectors_Configuration.DEBUG:
            print_exception('WARNING', Linio_Mex_config.MARKETPLACE, hirerachy, product_link, self.print_satement)

        return self.get_data(product_parameters, product_link, hirerachy, dirs), product_parameters['Date'], \
               product_parameters['Time']
