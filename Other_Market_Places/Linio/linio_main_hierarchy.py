...
def get_main_links():
    ...
    main_hierarchy_container = response_obj.get_content(Linio_Mex_config.MAIN_URL)
    categories = {}
    if main_hierarchy_container:
        ...
        if categories_container:
            ...
            for category_url in category_urls:
                url = '{}{}'.encode('utf-8').format(Linio_Mex_config.MAIN_URL,category_url.a['href'])
                category_name = category_url.a['title'].encode('utf-8').strip().replace(',','').replace(' ','_')
                urls_split = url.split('/')
                line = '{}|{}|{}|{}'.encode('utf-8').format(Linio_Mex_config.MARKETPLACE,Linio_Mex_config.MAIN_URL, category_name, url)
                if urls_split[-2] == 'c':
