...
class Workers:
    ...
    def name(self, url):
        ...
        if site:
            category = site.find_all('div', {'class': 'category_group_container'})[DataCollectors_Configuration.ANTIQUES_INDEX]
            main_category_name = site.find_all('div', {'class': 'category_group_container_mid'})[
                DataCollectors_Configuration.ANTIQUES_INDEX].find(
                'a').text
...
def start_hierarchy():
    ...
    print("antiques Hierarchy Started")
    start = time.time()
    workers_obj = Workers()
    ...
    print("Antiques Hierarchy " + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(
