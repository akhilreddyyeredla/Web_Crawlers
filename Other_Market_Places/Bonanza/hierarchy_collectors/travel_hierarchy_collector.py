...
class Workers:
    ...
    def name(self, url):
        ...
        if site:
            category = site.find_all('div', {'class': 'category_group_container'})[DataCollectors_Configuration.TRAVEL_INDEX]
            main_category_name = site.find_all('div', {'class': 'category_group_container_mid'})[
                DataCollectors_Configuration.TRAVEL_INDEX].find(
                'a').text
...
def start_hierarchy():
    ...
    print("TRAVEL Hierarchy Started")
    start = time.time()
    workers_obj = Workers()
    ...
    print("TRAVEL Hierarchy " + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(
