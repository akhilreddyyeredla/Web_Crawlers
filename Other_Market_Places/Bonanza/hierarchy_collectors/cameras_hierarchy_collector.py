...
class Workers:
    ...
    def name(self, url):
        ...
        if site:
            category = site.find_all('div', {'class': 'category_group_container'})[DataCollectors_Configuration.CAMERAS_INDEX]
            main_category_name = site.find_all('div', {'class': 'category_group_container_mid'})[
                DataCollectors_Configuration.CAMERAS_INDEX].find(
                'a').text
...
def start_hierarchy():
    ...
    print("CAMERAS Hierarchy Started")
    start = time.time()
    workers_obj = Workers()
    ...
    print("CAMERAS Hierarchy " + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(
