...
class Workers:
    ...
    def name(self, url):
        ...
        if site:
            category = site.find_all('div', {'class': 'category_group_container'})[DataCollectors_Configuration.HEALTH_BEAUTY_INDEX]
            main_category_name = site.find_all('div', {'class': 'category_group_container_mid'})[
                DataCollectors_Configuration.HEALTH_BEAUTY_INDEX].find(
                'a').text
...
def start_hierarchy():
    ...
    print("HEALTH_BEAUTY Hierarchy Started")
    start = time.time()
    workers_obj = Workers()
    ...
    print("HEALTH_BEAUTY Hierarchy " + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(
