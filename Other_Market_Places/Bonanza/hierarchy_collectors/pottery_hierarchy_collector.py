...
class Workers:
    ...
    def name(self, url):
        ...
        if site:
            category = site.find_all('div', {'class': 'category_group_container'})[DataCollectors_Configuration.POTTERY_INDEX]
            main_category_name = site.find_all('div', {'class': 'category_group_container_mid'})[
                DataCollectors_Configuration.POTTERY_INDEX].find(
                'a').text
...
def start_hierarchy():
    ...
    print("POTTERY Hierarchy Started")
    start = time.time()
    workers_obj = Workers()
    ...
    print("POTTERY Hierarchy " + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(
