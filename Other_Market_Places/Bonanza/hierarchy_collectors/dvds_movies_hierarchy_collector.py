...
class Workers:
    ...
    def name(self, url):
        ...
        if site:
            category = site.find_all('div', {'class': 'category_group_container'})[DataCollectors_Configuration.DVDS_MOVIES_INDEX]
            main_category_name = site.find_all('div', {'class': 'category_group_container_mid'})[
                DataCollectors_Configuration.DVDS_MOVIES_INDEX].find(
                'a').text
...
def start_hierarchy():
    ...
    print("DVDS_MOVIES Hierarchy Started")
    start = time.time()
    workers_obj = Workers()
    ...
    print("DVDS_MOVIES Hierarchy " + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(
