...
class Workers:
    ...
    def name(self, url):
        ...
        if site:
            category = site.find_all('div', {'class': 'category_group_container'})[DataCollectors_Configuration.EVERYTHING_ELSE_INDEX]
            main_category_name = site.find_all('div', {'class': 'category_group_container_mid'})[
                DataCollectors_Configuration.EVERYTHING_ELSE_INDEX].find(
                'a').text
...
def start_hierarchy():
    ...
    print("EVERYTHING_ELSE Hierarchy Started")
    start = time.time()
    workers_obj = Workers()
    ...
    print("EVERYTHING_ELSE Hierarchy " + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(
