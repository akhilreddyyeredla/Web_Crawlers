...
class Workers:
    ...
    def name(self, url):
        ...
        if site:
            category = site.find_all('div', {'class': 'category_group_container'})[DataCollectors_Configuration.COINS_INDEX]
            main_category_name = site.find_all('div', {'class': 'category_group_container_mid'})[
                DataCollectors_Configuration.COINS_INDEX].find(
                'a').text
...
def start_hierarchy():
    ...
    print("COINS Hierarchy " + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(
