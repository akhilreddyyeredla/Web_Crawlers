...
class Workers:
    ...
    def name(self, url):
        ...
        if site:
            category = site.find_all('div', {'class': 'category_group_container'})[DataCollectors_Configuration.COMPUTERS_INDEX]
            main_category_name = site.find_all('div', {'class': 'category_group_container_mid'})[
                DataCollectors_Configuration.COMPUTERS_INDEX].find(
                'a').text
...
def start_hierarchy():
    ...
    print("COMPUTERS Hierarchy Started")
    start = time.time()
    workers_obj = Workers()
    ...
    print("COMPUTERS Hierarchy " + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(
