...
class Workers:
    ...
    def name(self, url):
        ...
        if site:
            category = site.find_all('div', {'class': 'category_group_container'})[DataCollectors_Configuration.VIDEO_GAMES_INDEX]
            main_category_name = site.find_all('div', {'class': 'category_group_container_mid'})[
                DataCollectors_Configuration.VIDEO_GAMES_INDEX].find(
                'a').text
...
def start_hierarchy():
    ...
    print("VIDEO_GAMES Hierarchy Started")
    start = time.time()
    workers_obj = Workers()
    ...
    print("VIDEO_GAMES Hierarchy " + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(
