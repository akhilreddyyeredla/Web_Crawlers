...
class Workers:
    ...
    def name(self, url):
        ...
        if site:
            category = site.find_all('div', {'class': 'category_group_container'})[DataCollectors_Configuration.DIGITAL_GOODS_INDEX]
            main_category_name = site.find_all('div', {'class': 'category_group_container_mid'})[
                DataCollectors_Configuration.DIGITAL_GOODS_INDEX].find(
                'a').text
...
def start_hierarchy():
    ...
    print("DIGITAL_GOODS Hierarchy Started")
    start = time.time()
    workers_obj = Workers()
    ...
    print("DIGITAL_GOODS Hierarchy " + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(
