...
class Workers:
    ...
    def __init__(self, project_name, homepage, category_queue_file, sub_category_queue_file,
        ...
        print('starting accessories hierarchy collection')
    ...
    def crawl():
        ...
        if len(queued_links2) > 0:
            ...
            print('Total_time to collect accessories Hierarchy ' + str(total_time) + ' sec')
...
class Scrapper:
    ...
    def __init__(self, project_name, base_url, name):
        ...
        Scrapper.category_queue_file = Scrapper.project_name + DataCollectors_Configuration.CATEGORY_QUEUE.format(name)
        Scrapper.category_completed_file = Scrapper.project_name + DataCollectors_Configuration.CATEGORY_COMPLETED.format(name)
        Scrapper.sub_category_queue_file = Scrapper.project_name + DataCollectors_Configuration.SUB_CATEGORY_QUEUE.format(name)
        Scrapper.sub_category_completed_file = Scrapper.project_name + DataCollectors_Configuration.SUB_CATEGORY_COMPLETED.format(name)
        Scrapper.sub_sub_category_queue_file = Scrapper.project_name + DataCollectors_Configuration.SUB_SUB_CATEGORY_QUEUE.format(name)
        Scrapper.sub_sub_category_completed_file = Scrapper.project_name + DataCollectors_Configuration.SUB_CATEGORY_COMPLETED.format(
            name)
        Scrapper.skipped_queue_file = Scrapper.project_name + DataCollectors_Configuration.SKIPPED_QUEUE.format(name)
        Scrapper.skipped_completed_file = Scrapper.project_name + DataCollectors_Configuration.SKIPPED_COMPLETD.format(name)
        self.boot(name)
