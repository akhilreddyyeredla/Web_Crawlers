...
class Workers:
    ...
    def save_data(self,thread_name, hierarchy, url, last_page):
        ...
        if raw_data:
            ...
            productDetali_obj =books_movies_and_music_get_product_info.Product_Details()
...
def start_info_collection():
    starting_time = time.time()
    print('starting books_movies_and_music information collection')
    productinfo_obj = Workers()
    productinfo_obj.get_page_data()
    ending_time = time.time()
    total_time = ending_time - starting_time
    print('books_movies_and_music category products:' +"|"+"Start Time:"+str(starting_time)+"| Ending Time:"+str(ending_time)+"|"+"Total_time:"+ str(total_time) + ' sec')
