...
def collect_hirerachy_details():
    ...
    #create_project_dir(project_name_url)
    #create_project_dir(project_name_info)
    respose_obj =response_getter.Response()
    page_soup, recivied_url = respose_obj.get_page_soup(DataCollectors_Configuration.SOUQ_MAIN_URL)
...
def form_hierarchy(level):
    if level == 1:
        ...
        category_name = re.sub('[^a-zA-Z|]', '_', category_name).replace('__', '_').strip()
        ...
        if li2 in url_dict.keys():
            ...
            #create_project_dir(project_name_url + '/' + category_name.replace('|', '/'))
            #create_project_dir(project_name_info + '/' + category_name.replace('|', '/'))  
    elif level == 6:
        li = level_Dictionary[1][-1]
        li2 = level_Dictionary[4][-1]
        ...
        if li3 in url_dict.keys():
            ...
            category_name = re.sub('[^a-zA-Z|]', '_', category_name).replace('__', '_').strip()
            hierarchy_dict[category_name] = url_dict[li3]
            create_project_dir(project_name + '/' + category_name.replace('|', '/')).strip()
            #create_project_dir(project_name_url + '/' + category_name.replace('|', '/'))
            #create_project_dir(project_name_info + '/' + category_name.replace('|', '/'))      
    elif level == 8:
        li = level_Dictionary[1][-1]
