...
class workers:
    ...
    def mus(self,file):
        ...
        with open(file, 'rt') as f:
            ...
            self.urls.join()
        stop = time.time()
        print("Music" + "|" + "Start Time:" + str(start) + "|" + "End Time:" + str(stop) + "|" + "Total Duration:" + str(stop - start))
            ...
            if sub_url not in main_list:
                if sub_url not in urls_list:
                    ...
                    for leaf_data in leaf_container:
                        page_list = open(DataCollectors_Configuration.music_leaf.format(ROOT_FOLDER)).read().splitlines()
                        leaf_url = leaf_data['href'].encode('utf-8')
                        if leaf_url not in page_list:
                            if leaf_url not in leaf_list:
                                leaf_list.add(leaf_url)
                                leaf_name = leaf_data.text.replace(" ", "_").replace("&", "AND").replace(",", "_").encode('utf-8')
