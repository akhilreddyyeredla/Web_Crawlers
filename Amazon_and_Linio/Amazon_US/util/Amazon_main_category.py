...
from CONSTANTS import MAIN_PAGE_URL, DOMAIN_NAME
...
def collect_main_page_urls(main_url):
    ...
    category_containers = raw_data.findAll('div', {'class': 'fsdDeptBox'})

    for category in category_containers:
        category_name_tag = category.find('h2', {'class': 'fsdDeptTitle'})
        category_name = string_format(category_name_tag)
        print category_name
        urls_tag = category.findAll('a', {'class': 'a-link-normal fsdLink fsdDeptLink'})
        for url in urls_tag:
...
def create_hirerachy(starting_url):
    ...
    for category_name in category_hierarchy_and_urls.keys():
        category_path = '{}/{}'.format(PROJECT_NAME, category_name.replace('|', '/'))
...
create_hirerachy(MAIN_PAGE_URL)
